 ...
 *
 * $Source: /cvs_backup/e107_0.8/e107_plugins/efiction/e_sitelink.php,v $
 * $Revision$
 * $Date$
 * $Author$
 *
...
class efiction_sitelink // include plugin-folder in the name.
{
	...
	function myCategories()
	{
		...
		$sql->select("efiction","*","efiction_id != '' ");
		
		while($row = $sql->fetch())
		{
			$sublinks[] = array(
				'link_name'			=> $tp->toHTML($row['efiction_name'],'','TITLE'),
				'link_url'			=> e107::url('efiction', 'other', $row),
				'link_description'	=> '',
				'link_button'		=> $row['efiction_icon'],
				'link_category'		=> '',
				'link_order'		=> '',
				'link_parent'		=> '',
				'link_open'			=> '',
